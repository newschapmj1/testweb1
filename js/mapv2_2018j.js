...
var d1 = new Date();
var infoWindow;
var events;   //holds copy of each walk in array
var ui;
var offset =6;  //(1st month-1, eg July 6, Nov 10)
//var myVar;  //timer to set position of Slider when page loaded


//months of the walks (1..12)  eg 11,12,1,2,3
//use this to convert month into array 1..5
var activeMonth2 = [7,8,9,10,11];

function setUI2(month,position)
	{
	//"use strict";
	/* month ie to be evaluated 1-12, position start of list 1 -12
	eg Month Jan and position starts with Oct, output 4 as Jan is 4th month in list
	*/
	var result, adj;
    adj =  (12-position)+1;
	if (month < position)
...
function initMap1() {
    ...
    $('#date_slider').slider({
	    ...
	    //ui is the slider value 1 to 5
            //add on x to convert to months 1-12 eg if 1st Month is Nov add 10, then mod 12
            // Changing the label to the selected monthname
       	    newui = ((ui.value + 09) % 12);
            ...
            $( "#date_slider" ).slider( "option", "value", 3);
            //deleteOverlays();
            //drawevents(events, map, infoWindow, newui); called in initmap
            console.log('slider.slide end');
        },
...
console.log('slider.change')
    ...
    for (i = 0; i < events.length; i += 1) {
        ...
        events[i].RouteType = $(events[i]).find('.RouteType').html();
    ...
    //next line disabled 20180823 0806
    //setSlider();  //to current month
} //initmap
...
$(document).ready(function () {
    //commented out read  22 Aug 2018
    //"use strict";
  ...
  //$('#selected_month').html(monthName((d1.getMonth()) + 1,6)); //month 1-12
    $('#selected_month').html(monthName((d1.getMonth()) + 1)); //month 1-12

//getMonth returns 0-11
    //ui = setUI2((d1.getMonth()) +6,2); //month 1-12, eg 7 start slider in July
    //$('#date_slider').slider('value', ui);
    //$( ".selector" ).slider( "option", "value", 10 );
//    $('#date_slider').slider('value', ui);
    // filtering event list

    //next 2 lines commented out 23082018 08:00
    //filterEventList((d1.getMonth()) + 1); //
    //clearMap(); //commented 9/1/2011
    // filtering markers on map
    //clearMap();
    //filterMap((d1.getMonth() + 1)); // 9/1/2010
...
function setSlider()
{
    //move the slider to the current month
    console.log('setSlider started');
  
    //set to current month

var now       = new Date();
var thisMonth = now.getMonth() + 1; //month 0-11  will be converted to-> range on slider 1..5, 1..6 etc
var thisMonth2 = now.getMonth()+1; //1-12   
//Nov-Mar 1,5 offset 10  eg Dec 12-10=2,  Jan  1

//eg month eg 11,12,1,2,3  converted -> 1..5 ie nth position on slider
var newui= (activeMonth2.indexOf(thisMonth2))+1;

//var newui = (thisMonth2 - offset) % 12;
$('#selected_month').html(monthName( now.getMonth() + 1));
console.log('about to change slider to newui');
    $('#date_slider').slider('value',newui);
    $("#date_slider").slider("refresh");
    //ui 1..5,1..6
    //newui = ((ui.value + offset) % 12);
    console.log('newui (slider 1..5)',newui);
    console.log('thismonth2',thisMonth2);
    //clearMap();
    //filterEventList(now.getMonth() + 1);

    // filtering markers on map
    //filterMap(now.getMonth() + 1);
    console.log('setSlider ended');

    //$( "#date_slider" ).slider( "option", "value", thisMonth2);

    //$("#date_slider").slider('value',thisMonth2);
